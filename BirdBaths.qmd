...
Success! From this graph, we can see that the largest amount of birds were seen in the Sydney Basin, which makes up around 20% of all the observations. In comparison, South Eastern Queensland (SE Queensland) has the next largest amount of sightings with just over half the amount of birds (~11%). Brigalow Belt South and Victorian Volcano Plain were almost tied for the least amount of birds, only differing by 2 sightings. 
...
exploratory_data |>
  ...
  filter(bird_count > 0) |>
  ggplot() +
  geom_col(aes(x = bird_count, y = survey_year)) +
  labs(title = "Bird Count by Bioregion",
       x = "Number of Birds",
       y = "Survey Year") +
  guides(fill = "none")
